...
def _sample_incremental_ids(
    ...
    with sampler._lock:
        ...
        transition_ids, probabilities = _incremental_choices(sampler, request.batch_size)
        beta = sampler.beta if request.beta is None else request.beta
        result = _DrawResult(transition_ids, probabilities, beta)
...
def _incremental_choices(
    ...
    if sampler.expert_fraction == 0.0:
        return _draw_incremental_group(
            sampler, _GroupDraw(tree, uniform, sampler._active_count, batch_size, 1.0)
        )
    expert = _expert_group(sampler, batch_size, 1.0)
    if sampler._expert_active_count == sampler._active_count:
        return _draw_incremental_group(sampler, expert)
    return _stratified_incremental_choices(sampler, batch_size)
...
def _stratified_incremental_choices(
    ...
    expert_count = round(sampler.expert_fraction * batch_size)
    groups = (
