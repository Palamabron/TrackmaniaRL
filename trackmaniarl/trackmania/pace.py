...
class ReferencePaceProfile:
    ...
    def from_demonstration(
        ...
        *,
        ...
        return cls.from_frames(
            ...
            velocity_to_mps_scale=velocity_to_mps_scale,
        )
    ...
    def from_frames(
        ...
        *,
        ...
        if not np.isfinite(velocity_to_mps_scale) or velocity_to_mps_scale <= 0.0:
        ...
        sampled_speeds = (
            values[first_samples, 16] * velocity_to_mps_scale if values.shape[1] > 16 else None
        )
        speeds = (
            None
...
def _load_guidance_reference(path: str | Path, geometry: GeometryAsset) -> tuple[np.ndarray, float]:
    with np.load(Path(path), allow_pickle=False) as archive:
        ...
        missing = required - set(archive.files)
        if missing:
            raise ValueError(f"guidance reference is missing keys: {sorted(missing)}")
        if str(archive["map_uid"].item()) != geometry.map_uid:
