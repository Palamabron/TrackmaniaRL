...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--expert", action="store_true", help="launches an expert rollout worker (no model update)"
    )
    parser.add_argument("--test", action="store_true", help="runs inference without training")
    parser.add_argument(
        "--benchmark", action="store_true", help="runs a benchmark of the environment"
    )
    parser.add_argument(
        "--record-reward",
        dest="record_reward",
    ...
    parser.add_argument(
        ...
        dest="use_keyboard",
        action="store_true",
        help="modifier for --record-reward",
    )
    parser.add_argument(
        "-r",
        "--record-episode",
        dest="record_episode",
        action="store_true",
        help="utility to record an episode to store it in replay buffer in TM20",
    )
    parser.add_argument(
        "--check-environment",
        dest="check_env",
    ...
    parser.add_argument(
        ...
        dest="wandb",
        action="store_true",
        help="(use with --trainer) log results on Weights and Biases",
    )
    parser.add_argument(
        "-d",
        "--config",
