...
This script works out-of-the-box for Gymnasium environments with flat continuous
...
update_buffer_interval = (
    1  # the trainer checks for incoming samples at this interval of training steps
)
update_model_interval = (
    1  # the trainer broadcasts its updated model at this interval of training steps
)
max_training_steps_per_env_step = (
    0.2  # Trainer synchronization ratio (max training steps per collected env step)
)
...
def run_worker(worker):
    ...
    # initial_steps: env steps before waiting for the first model update.
    # max_steps_per_update: max env steps per model update (worker sync ratio).
    # end_episodes: wait for episodes to be terminated/truncated before syncing.
