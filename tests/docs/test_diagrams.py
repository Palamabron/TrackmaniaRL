...
RENDERER = runpy.run_path(str(DIAGRAMS / "render.py"), run_name="diagram_renderer")
...
RENDER_HTML = cast(
    Callable[[dict[str, Any], str, dict[str, Any], str], str], RENDERER["render_html"]
)
...
def test_diagram_spec_has_valid_references_and_unique_ids(stem: str) -> None:
    spec = _spec(stem)
...
@pytest.mark.parametrize("stem", EXPECTED_STEMS)
def test_committed_editable_and_web_previews_match_spec(stem: str) -> None:
    spec = _spec(stem)
    ...
    assert (DIAGRAMS / f"{stem}-preview.html").read_text(encoding="utf-8") == RENDER_HTML(
        spec, svg, scene, stem
    )
...
@pytest.mark.parametrize("stem", EXPECTED_STEMS)
def test_png_preview_matches_canvas(stem: str) -> None:
    spec = _spec(stem)
