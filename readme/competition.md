...
For the Alpha iteration, your score is evaluated on the `tmrl-test.Map.Gbx` track provided in `HOME_REDACTED\TmrlData\resources`.
...
The `"SLEEP_TIME_AT_RESET"` entry in `config.json` (`HOME_REDACTED\TmrlData\config`) is set to 0.0 to avoid wasting time at the beginning of the episode (but you should leave it to the default 1.5 for training, to alleviate non-Markovness).
