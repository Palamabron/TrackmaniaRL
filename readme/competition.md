...
The `"SLEEP_TIME_AT_RESET"` entry if `config.json` (`HOME_REDACTED\TmrlData\config`) is set to 0.0 to avoid wasting time at the beginning of the episode (but you should leave it to the default 1.5 for training, to alleviate non-Markovness).
