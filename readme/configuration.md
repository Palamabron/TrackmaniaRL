...
A RunSpec YAML file is the public runtime configuration boundary in 1.2.5.
...
| `start_race_time_ms` | `0` ms | Optional lower bound on the first observation after a detected restart. Timing alignment was not used in the V108 confirmation. |
...
The factories in `trackmaniarl.trackmania.actor_critic` are also exported from
`trackmaniarl.trackmania`. `TelemetrySacModelFactory`, `TelemetryRedqModelFactory`
and `TelemetryDiscreteSacModelFactory` accept `input_dim=33` and `hidden_dim=256`,
both positive. REDQ also accepts `critic_count=10`, at least two and not less than
the learner's target subset. Discrete SAC accepts `action_count=78`, at least two.
Changing this count requires exactly the same compact action table in the environment.
Each actor and critic owns independent encoder parameters. SAC and REDQ actors
bound gas and brake to `[0, 1]` and steering to `[-1, 1]`. These factories consume
`TelemetryFeaturePipeline` output, not lidar mappings or camera stacks.

PPO uses local on-policy collection with `OnPolicySequenceSampler`. Set `n_step: 1`,
choose a fixed rollout `sequence_length` and make `total_transitions` divisible by
that length. The replay store capacity must hold a full rollout. `batch_size: 1`
describes the rollout batch, while `minibatch_size` below controls optimizer steps.
The on-policy trainer updates once per fresh rollout, so `warmup_transitions`, PER
and `updates_per_transition` do not schedule PPO updates.
...
The [vision guide](vision.md) explains the complete frame and PPO update flow.
These component options configure the image path. Defaults are library defaults.

| Component and option | Default | Meaning |
| --- | --- | --- |
| `VisionFeaturePipeline.kwargs.config.width` / `height` | `84` / `84` | Resized image dimensions, each at least 8 pixels |
| `VisionFeaturePipeline.kwargs.config.grayscale` | `true` | Convert RGB to one luminance channel before stacking |
| `VisionFeaturePipeline.kwargs.config.frame_stack` | `4` | Consecutive images concatenated into channels, at least one |
| `VisionPpoModelFactory.kwargs.channels` | `4` | Must equal frame count times channels per frame |
| `VisionPpoModelFactory.kwargs.hidden_dim` | `256` | CNN embedding width for actor and value branches |
| `VisionEnvironmentFactory.kwargs.config` | required | Standard Trackmania environment and reward options |
| `VisionEnvironmentFactory.kwargs.capture.left` / `top` | `0` / `0` | Desktop rectangle origin in pixels, negative coordinates are allowed |
| `VisionEnvironmentFactory.kwargs.capture.width` / `height` | `1280` / `720` | Positive capture dimensions before resizing |

The bundled vision PPO configuration sets `normalize_observations: false` because
the feature pipeline already scales pixels to `[0,1]`. Set model channels together
with grayscale and frame-stack settings. Offline validation catches model/input
shape mismatches without opening a screen capture session.
...
Optional `AdaptiveGradientClipper` is applied after AMP unscaling and
...
`clip_factor=2.0` (positive). Optional
...
setups. Change one configuration variable at a time and compare against an
